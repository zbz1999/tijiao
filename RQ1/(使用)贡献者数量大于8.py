...
def get_contributors_count(repo_name):
    ...
    if response.status_code == 200:
        return len(response.json())  # 返回贡献者的数量
    else:
        return 0
...
def process_url(url):
    ...
    contributors_count = get_contributors_count(repo_name)
    return url, contributors_count >= 8  # 检查是否至少有8个贡献者

# 主函数
def main():
    # 输入和输出文件路径
    input_file = 'H:/filtered_projects_issues.xlsx'
    output_file_filtered = 'H://filtered_projects_contributors.xlsx'
    output_file_removed = 'H://removed_projects_contributors.xlsx'
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_url):
            url, has_enough_contributors = future.result()
            if has_enough_contributors:
