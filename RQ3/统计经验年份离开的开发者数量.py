...
output_folder = "H:/1_合并RQ3.2/fenkaide_all_output_csvs"  # 结果输出目录
...
for file_name in os.listdir(input_folder):
    if file_name.endswith(".csv"):
        ...
        # 统计每种活跃年数的开发者数量
        year_counts = df["Active Years"].value_counts().to_dict()

        # 转换为 DataFrame
        result_df = pd.DataFrame(sorted(year_counts.items()), columns=["Year", "Number of Developers"])

        # 生成单独的输出文件名
        output_file = os.path.join(output_folder, f"{os.path.splitext(file_name)[0]}_years.csv")
