...
# 获取Git仓库路径
def get_git_repos(root_folder, start_from=None):
    repos = [os.path.join(root_folder, d) for d in sorted(os.listdir(root_folder))
             if os.path.isdir(os.path.join(root_folder, d))]

    # 如果指定了起始仓库，跳过之前的仓库
    if start_from:
        if start_from in repos:
            return repos[repos.index(start_from):]
        else:
            print(f"指定的起始仓库 {start_from} 不存在，将从头开始")
    return repos
    ...
    for line in log_output.splitlines():
        ...
        if '<' in author_part and '>' in author_part:
            try:
                ...
                print(f"解析作者信息出错，跳过: {author_part}")
                continue
        else:
            print(f"跳过无效的作者信息: {author_part}")
            continue
...
def main():
    ...
    start_from_repo = os.path.join(input_root_folder, 'kotlin')

    for repo in get_git_repos(input_root_folder, start_from_repo):
