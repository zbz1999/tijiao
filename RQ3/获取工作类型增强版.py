...
start_from_repo = 'kotlin'  # 从哪个仓库开始处理（None表示从第一个开始）
...
def process_all_repos_in_folder(folder_path, output_folder, start_from=None):
    """批量处理所有仓库"""
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    repo_list = sorted([repo for repo in os.listdir(folder_path)
                        if os.path.isdir(os.path.join(folder_path, repo))])

    start_processing = start_from is None  # 如果没指定起始仓库，则从第一个开始
    ...
    for repo_name in repo_list:
        ...
        if not start_processing:
            if repo_name.lower() == start_from.lower():
                start_processing = True
            else:
                print(f"跳过仓库: {repo_name} (等待到达起始仓库 {start_from})")
                continue
...
if __name__ == "__main__":
    ...
    process_all_repos_in_folder(input_folder, output_folder, start_from=start_from_repo)
